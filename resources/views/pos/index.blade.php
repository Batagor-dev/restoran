...
<div class="pos-container" x-data="posApp()" x-init="init()">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            {{-- Toolbar --}}
            <div class="flex items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                <div class="flex items-center gap-3 flex-1">
                    <div class="relative flex-1">
                        <i class=" absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"></i>
                        <input type="text"
                            x-model="search"
                            placeholder="Search product..."
                            class="w-full pl-9 pr-4 py-2 rounded-lg border-slate-300 focus:border-slate-500 focus:ring-1 focus:ring-slate-500">
                    </div>
                    <select x-model="category" @change="filterProducts()"
                        class="px-4 py-2 rounded-lg border-slate-300 focus:border-slate-500 focus:ring-1 focus:ring-slate-500">
            ...
            {{-- Product Grid --}}
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 max-h-[calc(100vh-300px)] overflow-y-auto p-1">
                    ...
                    <div @click="addToCart(product)"
                        ...
                        <div class="relative aspect-square bg-slate-100 rounded-t-xl overflow-hidden">
                            ...
                            <button @click.stop="toggleFavorite(product)"
                                ...
                                <i class="text-lg" :class="product.is_favorite ? 'ri-star-fill text-yellow-400' : 'ri-star-line text-slate-400'"></i>
                            </button>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button @click="clearCart()" class="text-xs text-red-500 hover:text-red-700 transition" x-show="cart.length > 0">
                            Clear All
                        </button>
                    </div>
                </div>
                    ...
                    <template x-for="(item, index) in cart" :key="index">
                        <div class="flex items-center gap-3 p-2 bg-slate-50 rounded-lg">
                            ...
                            <div class="flex items-center gap-1">
                                <button @click="updateCartItem(index, -1)" class="w-6 h-6 rounded bg-slate-200 hover:bg-slate-300 flex items-center justify-center text-sm">-</button>
                ...
                <div class="mt-4 pt-4 border-t border-slate-200 space-y-2">
                    ...
                    <div class="flex justify-between text-sm" x-show="discount > 0">
                        ...
                        <span class="font-satoshi-medium text-green-600">-Rp <span x-text="formatNumber(discount)"></span></span>
                    </div>
                ...
                <div class="mt-3 flex gap-2">
                    <input type="text"
                        ...
                        :disabled="discount > 0"
                        class="flex-1 px-3 py-1.5 text-sm rounded-lg border-slate-300 focus:border-slate-500 focus:ring-1 focus:ring-slate-500 disabled:bg-slate-100 disabled:text-slate-500">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
                ...
                <select x-model="customerId"
                    class="w-full px-3 py-2 text-sm rounded-lg border-slate-300 focus:border-slate-500 focus:ring-1 focus:ring-slate-500">
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('posApp', () => ({
            ...
            addToCart(product) {
                ...
                if (price <= 0) {
                    alert('Product price is 0 or invalid!');
                    return;
                }
                ...
                fetch('/pos/cart/add', {
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            ...
                            alert('Gagal menambahkan ke cart: ' + (data.message || 'Unknown error'));
                        }
                    })
                    .catch(error => {
                ...
                fetch('/pos/promo/apply', {
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            ...
                            this.promoMessage =
                                data.scope_message || 'Promo berhasil diterapkan!';
                            ...
                            // Promo tidak ditemukan / expired
                            alert(
                                data.message ||
                                'Kode promo tidak ditemukan atau sudah tidak berlaku.'
                            );

                            // Jangan mengubah discount di sini
                            // supaya promo yang sedang aktif tidak ikut terhapus
                        }
                    })
                    .catch(error => {
                        ...
                        alert(
                            'Terjadi kesalahan saat memeriksa kode promo.'
                        );
                    });
            },
            ...
            processOrder() {
                ...
                // Ambil nama customer dari dropdown
                const customerSelect = document.querySelector('select[x-model="customerId"]');
                ...
                // Mapping cart items dengan notes
                ...
                fetch('/pos/order', {
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            alert('Order processed successfully! Invoice: ' + data.invoice);
                            this.clearCart();
                            this.customerId = '';
                            this.tableId = '';
