...
class OutletDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addIndexColumn()
            ->addColumn('status', fn ($outlet) => $outlet->status
            ...
            ->addColumn('action', function ($row) {
                ...
                if (auth()->user()->can('Outlet Update')) {
                    $edit = '<a href="'.route('outlet.edit', $row->uuid).'" 
                ...
                if (auth()->user()->can('Outlet Delete')) {
                    $delete = '
                            <form action="'.route('outlet.destroy', $row->uuid).'" method="POST" style="display:inline-block;" class="delete-form m-0">
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('name')->title('Outlet Name')->addClass('px-4 py-3 border-b border-slate-200 text-slate-900 font-semibold'),
            Column::make('phone')->title('Phone')->addClass('px-4 py-3 border-b border-slate-200 text-slate-500'),
            Column::make('address')->title('Address')->addClass('px-4 py-3 border-b border-slate-200 text-slate-500'),
