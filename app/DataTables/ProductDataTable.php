...
class ProductDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                if (auth()->user()->can('Product Update')) {
                    $edit = '<a href="'.route('products.edit', $row->uuid).'"
                ...
                if (auth()->user()->can('Product Delete')) {
                    $delete = '
                        <form action="'.route('products.destroy', $row->uuid).'"
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('image')->title('Image')->searchable(false)->orderable(false)->addClass('text-center px-4 py-3 border-b border-slate-200'),
            Column::make('name')->title('Product Name')->addClass('px-4 py-3 border-b border-slate-200 text-slate-900 font-semibold'),
            Column::make('category_name')->title('Category')->orderable(false)->addClass('px-4 py-3 border-b border-slate-200 text-slate-700'),
            Column::make('price')->title('Price')->addClass('px-4 py-3 border-b border-slate-200 text-slate-900 font-semibold'),
